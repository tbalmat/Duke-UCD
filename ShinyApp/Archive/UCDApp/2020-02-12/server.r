...
assembleNetComponents <- function(pConcept, nodeVar1, nodeVar2, nedgemin, eopacity) {
  ...
  if(nrow(pConcept)>0) {
    # Tabulate edges by node vars 1 an 2
...
composeNet <- function(nodeVar1, nodeVar2, vertex, edge, nCluster) {

  g <- visNetwork(vertex, edge) %>% 
         visGroups(groupname=nodeVar1, color=vc1, font=list("color"="white", "size"=12)) %>%
         visGroups(groupname=nodeVar2, color=vc2, font=list("color"="#202020", "size"=12)) %>%
  ...
  function(input, output, session) {
    ...
    observeEvent(input$renderGraph,{
      ...
      query <- paste(" match(x:Participant)-[:P_SCT]-(y:ObjectConcept)-[:ISA]-(z:ObjectConcept)",
                     ...
                     " return case when(labels(x)=['Participant'])then 'Participant'",
      ...
      #print(pConcept)
      netComponents <- assembleNetComponents(pConcept, "FSN", input$nodeVar, input$nedgemin, input$eopacity)
      #print(netComponents)
      # Render graph
      if(nrow(netComponents[["vertex"]])>0) {
        ...
        output$g1 <- renderVisNetwork(composeNet("FSN", input$nodeVar, netComponents[["vertex"]], netComponents[["edge"]], input$nCluster))
